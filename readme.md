...
> ⚠️ **Troubleshooting:** If `npm install` fails due to dependency conflicts (common with older Node versions or complex projects), use the `--legacy-peer-deps` flag:
>
> ```bash
...
The server will be running at `http://localhost:5000`. You can test the API endpoints using tools like Postman or by connecting your frontend application.
